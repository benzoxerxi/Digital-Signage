...
@login_required
def get_playlist():
    """Get current playlist with video metadata for logged-in user"""
...
def get_video(filename):
    """Serve video file - supports both authenticated users and user_id parameter"""
    ...
    # For Android APK - Support user_id parameter
    user_id = request.args.get('user_id')
    
    # Default to user_id=1 (admin) for backward compatibility
    if not user_id:
        user_id = 1
        print(f"⚠️  No user_id in video request, defaulting to user_id=1")
...
def get_playback_state():
    ...
    # For Android APK - Support user_id parameter for backward compatibility
    user_id = request.args.get('user_id')
    
    # If no user_id provided, use default (admin = user 1) for backward compatibility
    if not user_id:
        user_id = 1  # Default to admin user
        print(f"⚠️  No user_id provided, using default user_id=1")
    ...
    try:
        ...
        return jsonify({
            ...
            'message': f'user_id must be a number, got: {user_id}'
        }), 400
    except Exception as e:
        return jsonify({
    ...
    try:
        ...
        if not user.is_subscription_active():
            return jsonify({
                ...
                'message': f'User {user_id} subscription is not active. Current status: {user.subscription_status}'
            }), 403
    except Exception as e:
        return jsonify({
    ...
    try:
        ...
        if device_data.get('current_video'):
            response = {
                ...
                'mode': 'auto' if len(playlist.get('videos', [])) > 1 else 'manual',
                'last_update': device_data.get('last_seen'),
                'video_url': f'/api/video/{device_data["current_video"]}?user_id={user_id}',
                'device_id': device_id,
                'loop': playlist.get('settings', {}).get('loop', True),
                'interval': playlist.get('settings', {}).get('interval', 30)
            }
...
def next_video():
    ...
    # Get user_id (default to 1 for backward compatibility)
    user_id = request.args.get('user_id')
    if not user_id:
        if current_user.is_authenticated:
            ...
            user_id = 1
            print(f"⚠️  No user_id in next request, defaulting to user_id=1")
    else:
        try:
            user_id = int(user_id)
        except:
    ...
    if device_id not in devices:
        ...
        return jsonify({
            ...
            'video_url': f'/api/video/{playlist["videos"][0]["filename"]}?user_id={user_id}'
        })
    ...
    return jsonify({
        ...
        'total_videos': len(playlist['videos']),
        'video_url': f'/api/video/{next_video}?user_id={user_id}',
        'loop': playlist.get('settings', {}).get('loop', True)
    })
...
def get_analytics():
    ...
    stats = {
        ...
        'storage_used_gb': round(get_storage_usage(), 2),
